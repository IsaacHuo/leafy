...
`SUPABASE_CAMPUS_AI_FUNCTION` 为空时默认使用 `campus-ai-assistant`。当前客户端 CampusAI 入口固定直连 DeepSeek，模型与 Base URL 由 App 内置，用户只需在设置中填写 API Key。API Key 仅保存在当前设备的 Keychain，不通过 `xcconfig` 或 Info.plist 注入。
...
35. `20260605005826_campus_scope_v1.sql`
36. `20260625053303_campus_ai_usage_events.sql`
...
`campus-ai-assistant` 要求调用方携带有效 Supabase Auth JWT。函数以 `text/event-stream` 返回 Leafy 归一化后的 `delta` / `done` / `error` 事件，内部使用 OpenRouter Chat Completions `stream: true` 生成 Markdown 回复；不保存 prompt 或 response 正文。`private.campus_ai_usage_events` 只记录用户、校园、模型、状态、字符数、动作数和错误码，用于限流与排障。当前 App 内 CampusAI 不调用这个 Edge Function，而是使用用户保存在本机 Keychain 的 DeepSeek API Key 直连供应商；函数部署配置保持不变。部署前需要设置：

```bash
supabase secrets set OPENROUTER_API_KEY=...
supabase secrets set OPENROUTER_CAMPUS_AI_MODEL=openrouter/owl-alpha
```
